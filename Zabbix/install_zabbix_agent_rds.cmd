...
:: ============================================================
::  PASO 3 - Crear carpetas de trabajo
:: ============================================================
echo [PASO 3] Creando carpetas de trabajo...
mkdir "C:\app\zabbix\scripts" 2>nul
mkdir "C:\app\zabbix\conf\rdp" 2>nul
echo [OK] Carpetas creadas.

:: ============================================================
::  PASO 4 - Descargar archivos de configuracion RDP
:: ============================================================
echo [PASO 4] Descargando archivos de configuracion RDP...

curl -s "https://files.jezz.systems/etc/zabbix/terminal/rdp.terminal.server.ps1" -o "C:\app\zabbix\scripts\rdp.terminal.server.ps1"
IF NOT EXIST "C:\app\zabbix\scripts\rdp.terminal.server.ps1" (
    echo [ERROR] No se pudo descargar rdp.terminal.server.ps1
    goto :ERROR
)

curl -s "https://files.jezz.systems/etc/zabbix/terminal/rdp.terminal.server.conf" -o "C:\app\zabbix\conf\rdp\rdp.terminal.server.conf"
IF NOT EXIST "C:\app\zabbix\conf\rdp\rdp.terminal.server.conf" (
    echo [ERROR] No se pudo descargar rdp.terminal.server.conf
    goto :ERROR
)

curl -s "https://files.jezz.systems/etc/zabbix/terminal/PSTerminalServices.zip" -o "C:\app\zabbix\conf\rdp\PSTerminalServices.zip"
IF NOT EXIST "C:\app\zabbix\conf\rdp\PSTerminalServices.zip" (
    echo [ERROR] No se pudo descargar PSTerminalServices.zip
    goto :ERROR
)
echo [OK] Archivos descargados.

:: ============================================================
::  PASO 5 - Instalar modulo PowerShell
:: ============================================================
echo [PASO 5] Instalando modulo PSTerminalServices...
powershell -command "Expand-Archive 'C:\app\zabbix\conf\rdp\PSTerminalServices.zip' -DestinationPath 'C:\Windows\System32\WindowsPowerShell\v1.0\Modules\' -Force"
IF %ERRORLEVEL% NEQ 0 (
    echo [ERROR] No se pudo extraer PSTerminalServices.zip
    goto :ERROR
)
del "C:\app\zabbix\conf\rdp\PSTerminalServices.zip" >nul 2>&1
echo [OK] Modulo instalado.

:: ============================================================
::  PASO 6 - Agregar parametros al conf
:: ============================================================
echo [PASO 6] Configurando zabbix_agentd.conf...
SET CONFFILE=%INSTALLFOLDER%\zabbix_agent2.conf

:: Verificar que el conf existe
...
findstr /C:"rdp.terminal.server" "%CONFFILE%" >nul 2>&1
IF %ERRORLEVEL% NEQ 0 (
    echo UserParameter=rdp.terminal.server[*],powershell.exe -NoProfile -File "C:\app\zabbix\scripts\rdp.terminal.server.ps1" "$1" "$2" >> "%CONFFILE%"
)
...
:: ============================================================
::  PASO 7 - Configurar reinicio automatico del servicio
:: ============================================================
echo [PASO 7] Configurando reinicio automatico del servicio...
...
:: ============================================================
echo [PASO 8] Iniciando servicio Zabbix Agent 2...
