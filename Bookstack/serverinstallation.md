...
- [ ] suse server review: is it necessary to update suse? `zypper sl;cat /etc/issue;cat /etc/os-release`
- [ ] suse server prepare: make sure you are able to update sles, see 'sles_updates', 'sles_updates_prep'
...
- [ ] "ALTER SYSTEM STOP DATABASE <database_name>" on multi tenant as SYSTEM@SYSTEMDB
...
- [ ] Configure SNMP to Monitor with Zabbix
...
- [ ] create DC ( 1xCPU, 4GB RAM, 40GB HDD/SSD Thick, ISO )
- [ ] create ADM ( 2xCPU, 8GB RAM, 60GB HDD/SSD Thick, ISO )
- [ ] create RDS ( 4xCPU, 16GB RAM, 80GB HDD/SSD Thick, ISO )
...
- [ ] server Manager: hostname, diskmgmt, lusrmgr administrator PW (Password never expires), IPv4 Config, IPv6 off, IE-security settings, RDP, File Sharing, file extension and hidden elements, privat network (secpol.msc > security settings > network-list-manager-guidelines), green dasboard, time, energy options maximum power, screen never off, reboot
- [ ] ChocolateyStd: `choco install -y googlechrome notepadplusplus 7zip choco-upgrade-all-at`
- [ ] Anydesk: unattended with pw: `Tarantu95(ad)` then reboot (only Customer Site)
- [ ] install SAP B1: for terminal server & workstations install HDBClient&B1Client using `SAP.PS1`
- [ ] install SAP B1: for admin server install HDBClient&B1Client&HANAStudio&B1Components, use setup wizard to install SDK,DTW,B1i (B1i="Integration Solution Components") + Crystal Reports see the correct version here SAP NOTE: [2329487 - Crystal Reports for SAP Business One Matrix](https://me.sap.com/notes/2329487). RSP and Crystal Reports needs to be downloaded in the SAP Portal (https://me.sap.com/softwarecenter)
- [ ] Disable SAP Client Agent ```sc stop "SBOClientAgent" && sc config "SBOClientAgent" start= disabled```
- [ ] RDP login check
...
- [ ] install zabbix on all servers (check SUSE Version, needs 15.2 or higher) https://bookstack.isystems-integration.com/books/zabbix

- [ ] install Action1 on all Windows Servers https://bookstack.isystems-integration.com/books/action1/page/installations-done-by-webshop

  - [ ] install sapb1servertools script
...
- [ ] move the iso to your downloads folder and restart the windows server
  **Actual Path 16/07/2024** `C:\Users\Administrator\AppData\Local\Temp\chocolatey\sql-server-2019\15.0.2000.20210324\SQLServer2019-x64-ENU-Dev.iso`
...
- [ ] While SQL Installation change paths (SQL Server = Disk App // Databases = Disk Data // Logs = Disk Log // Temp = Disk App)
- [ ] change the password and set sql login mode for sa user: see below **win_sql**
- [ ] set Lock Pages in Memory on 64-bit SQL Server ( sap note 1134345 ): gpedit.msc > Local Computer Policy > Computer Configuration > Windows Settings > Security Settings > Local Policies > User Rights Assignments > Lock pages in Memory > User = 'NT Service\MSSQLSERVER' --> **Actual User**
- [ ] allow connections by setting the firewall: `netsh advfirewall firewall add rule name=SAP protocol=TCP dir=in localport=80,443,1433,7299,8080,8100,8443,40000,40020,40021,50000 action=allow`
...
- [ ] set LicenseServer IPv4:
`Set-Itemproperty -Path "HKLM:\SOFTWARE\WOW6432Node\ACE\TAO" -Name "TaoNamingServiceOptions" -value "-ORBEndPoint iiop://127.0.0.1:30000 -ORBDottedDecimalAddresses 1"`
**New Path** `Set-Itemproperty -Path "HKLM:\SOFTWARE\ACE\TAO" -Name "TaoNamingServiceOptions" -value "-ORBEndPoint iiop://127.0.0.1:30000 -ORBDottedDecimalAddresses 1"`
...
- [ ] SUSEConnect -r 133713371337 -e [support_eu@isystems-integration.com](mailto:support_eu@isystems-integration.com)
- [ ] install cronjob if ServiceLayer is Installed `15  1-21/3 * * * sync ; echo 1 > /proc/sys/vm/drop_caches ;`
...
- [ ] when partner is Part, always Install API Gateway Service and EDS
...
Boockstack > iSystems > Windows > Certificates > Generate Certificate with pki
...
#gatewayhostname:s:rds.quooder.privatcloud.biz:10235
...
net stop SAPB1iDIProxy_Monitor && net stop SAPB1iDIProxy && net stop SAPB1iEventSender && net stop Tomcat8
net start Tomcat8 && net start SAPB1iEventSender && net start SAPB1iDIProxy && net start SAPB1iDIProxy_Monitor
```
...
```powershell
Add-MpPreference -ExclusionPath "c:\program files\sap","c:\program files (x86)\sap","c:\programdata\sap","c:\program files\microsoft sql server","c:\program files (x86)\microsoft sql server","c:\windows\inf","c:\windows\system32\spool"
Add-MpPreference -ExclusionProcess "cks.startup.exe","conship.ui.exe","coresuite.exe","sbo.bab.exe","sbo.prd.exe","b1browser.exe","sap business one.exe","sap business one (x86).exe","explorer.exe","rdpclip.exe","runtimebroker.exe","spoolsv.exe","vmtoolsd.exe","agentconsole.exe","agentservice.exe","b1clientagent.exe","sbo.app.backend.exe","sbo.app.exe","dtw.exe","sapbouicom.exe","httpd.exe","tomcat7.exe","sqlbrowser.exe","sqlceip.exe","sqlservr.exe","sqlwriter.exe","qemu-ga.exe","blnsvr.exe"
...
#slesRegcode="4EC463523AFF52EC";#2024-01-27
...
8) Run size of tables on disk check to
